...
import de.maksn.kasseadvanced.Taste;
...
public class MySQLiteHelper extends SQLiteOpenHelper{
	...
	public int increaseKundenItem(String artikel, float ezpreis, int kid){
	    ...
	    Cursor cursor =
	            db.query(TABLE_NAME, // a. table
	            COLUMNS, // b. column names
	            " artikel = ? AND kunden_id = ?", // c. selections
	            new String[] { artikel, String.valueOf(kid) }, // d. selections args
	            null, // e. group by
	            null, // f. having
	            null, // g. order by
	            null); // h. limit
	...
	public void deleteAllKundenItems(int kid){
		...
		db.setLocale(Locale.getDefault());
		db.setLockingEnabled(true);
		String table = "kundenselecteditems";
		String whereClause = "kunden_id = ?";
		String whereArgs[] = { String.valueOf(kid) };
		db.delete(table, whereClause, whereArgs);
		
		Log.d("SQL","deleteAllKundenItems erfolgreich");
	...
	public Cursor alleKunden(){
		...
		String columns[] ={"vorname", "nachname", "betrag", "id"};
		String table = "kunden";
		String orderBy = "nachname, vorname ASC";
	...
	public Taste getButtonValues(Taste taste, int btnid){
		...
		SQLiteDatabase db = this.getReadableDatabase();
		String table = "tasten";
		String selection = "btn_id = ?";
		String selectionArgs[] = { String.valueOf(taste.getId()) };
		String columns[] = {"artikelname", "preis", "farbe"};
    ...
    public String getButtonColor(int btnid){
    	...
    	String selection = "btn_id = ?";
    	String selectionArgs[] = { String.valueOf(btnid) }; 
    	Cursor cursor = db.query(table, columns, selection, selectionArgs, null, null, null);
    	String farbe;
		if (cursor.moveToFirst()){
...
public double getActKundenGesmatBetrag(int kid){
	...
	public Cursor getAllUmsaetzeSumme(){
		...
		String selection = "anzahl > ?";
		String selectionArgs[] = {"0"};
		 
	    SQLiteDatabase db = this.getReadableDatabase();
	    // db.query(TABLE_NAME, COLUMNS, selection, selectionArgs, GROUPBY, null, null);
	...
	public Cursor getKundenNameAsCursor(int kid) {
		...
		String selection = "id = ?";
		String selectionArgs[] = {String.valueOf(kid)};
		Cursor cursor = sqldb.query(table, columns, selection, selectionArgs, null, null, null);
	...
	public long getStartUmsaetze() {
		...
		SQLiteDatabase db = this.getReadableDatabase();
		String table = "umsaetze";
		String columns[] = {"vdate"};
		String orderBy = "vdate ASC";
		String limit = "1";
		db.query(table, columns, null, null, null, null, orderBy,limit);
	...
	public int increaseValueKundenItem(String streArtikel, double ePreis, int eMenge, int kid) {
		...
		String columns[] = { "artikel", "anzahl"};
		String table = "kundenselecteditems";
		String selection = "artikel = ? AND kunden_id = ?";
		String selectionArgs[] = {streArtikel, String.valueOf(kid)}; 
		Cursor cursor =  db.query(table, columns, selection, selectionArgs, null, null, null);
		//String query = "SELECT artikel, anzahl FROM kundenselecteditems WHERE artikel = '"+streArtikel+"' AND kunden_id = '"+kid+"'";
		Log.d(tag, "increaseValueKundenItems");
		if (!cursor.moveToFirst()){
			// Wenn Artikel noch nicht vorhanden dann erstellen und auf Anzahl auf 1 
						ContentValues values = new ContentValues();
			...
			ContentValues values = new ContentValues();
			values.put("anzahl", anzahl);
			String selection2 = "artikel = ? AND kunden_id = ?";
			String selectionArgs2[] = {streArtikel, String.valueOf(kid)};
			db.update(table, values, selection2, selectionArgs2);
			Log.d(tag, "UPDATE ITEMSZAHLKUNDE");
	...
	public int increaseValueItem(String streArtikel, double ePreis, int eMenge) {
		...
		String columns[] = {"artikel", "anzahl" };
		String table = "selecteditems";
		String selection = "artikel = ?";
		String selectionArgs[] = { streArtikel };
		Cursor cursor = db.query(table, columns, selection, selectionArgs, null, null, null);
		
		Log.d(tag, "increaseValue");
		if (!cursor.moveToFirst()){
			// Wenn Artikel noch nicht vorhanden dann erstellen und auf Anzahl auf 1 
						ContentValues values = new ContentValues();
						//Log.d("addButton", String.valueOf(btnid));
